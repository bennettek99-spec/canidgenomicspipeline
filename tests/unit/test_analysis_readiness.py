...
def test_readiness_filters_non_autosomes_and_emits_small_population_warning(
    ...
    store = save_genotypes(
        ...
        Genotypes(
            allel.GenotypeArray(gt), np.array([100, 200, 300]),
            np.array(["1", "1", "X"]), np.array(["A", "B", "C", "D"]),
        ),
    )
    ...
    tmp_context.datastore.register(
        Artifact(
            ArtifactKind.GENOTYPES, "genotypes", store, FileFormat.NPZ,
    ...
    audit = json.loads(
        tmp_context.datastore.get(ArtifactKind.QC_TABLE, "analysis_readiness")
        .path.read_text(encoding="utf-8")
    )
    assert audit["status"] == "warning"
    assert {issue["code"] for issue in audit["issues"]} >= {
        "small_population", "ascertainment_panel_relative"
    }
