...
def test_breed_ranking_barplot_writes_a_png(tmp_path: Path) -> None:
    ...
    pd.DataFrame([
        {"sample_id": "NYFIXTURE_F1", "rank": 1, "breed": "Beagle",
         "log_likelihood": -10.0, "gap_to_next": 12.0, "loci_used": 285},
        {"sample_id": "NYFIXTURE_F1", "rank": 2, "breed": "LabradorRetriever",
         "log_likelihood": -22.0, "gap_to_next": None, "loci_used": 285},
        {"sample_id": "NYFIXTURE_OFF1", "rank": 1, "breed": "Beagle",
         "log_likelihood": -30.0, "gap_to_next": 0.5, "loci_used": 285},
...
def test_breed_ranking_barplot_handles_empty_scores(tmp_path: Path) -> None:
    ...
    pd.DataFrame(columns=["sample_id", "rank", "breed", "log_likelihood"]).to_csv(
        scores, index=False)
    primary = tmp_path / "breed_assign.csv"
    pd.DataFrame(columns=["sample_id", "any_dog_log_likelihood"]).to_csv(
        primary, index=False)
    out = figures.breed_ranking_barplot(scores, primary, tmp_path / "breed_ranking.png")
    assert _png(out)
...
def test_admixture_scatter_writes_a_png(tmp_path: Path) -> None:
    ...
    pd.DataFrame([
        {
            "sample_id": "Coyote001", "region": "western",
            "f_wolf": 0.0, "f_dog": 0.0,
            "f_wolf_ci_lo": 0.0, "f_wolf_ci_hi": 0.0,
            "f_dog_ci_lo": 0.0, "f_dog_ci_hi": 0.0,
        },
        {
            "sample_id": "Coyote168", "region": "eastern",
            "f_wolf": 0.11, "f_dog": 0.0,
            "f_wolf_ci_lo": 0.0, "f_wolf_ci_hi": 0.2,
            "f_dog_ci_lo": 0.0, "f_dog_ci_hi": 0.05,
        },
    ]).to_csv(csv, index=False)
