...
def harmonized(tmp_path_factory) -> GlobalConfig:
    ...
    cfg = GlobalConfig.load(
        overrides={
            "project_name": "harm",
            "paths.root": str(ws),
            "pipeline": ["harmonize", "load_genotypes", "pca"],
            ...
            "stages": {
                "harmonize": {
                    "datasets": [
                        {
                            "vcf": str(parts["A"][0]),
                            "sample_sheet": str(parts["A"][1]),
                            "dataset_id": "studyA",
                        },
                        {
                            "vcf": str(parts["B"][0]),
                            "sample_sheet": str(parts["B"][1]),
                            "dataset_id": "studyB",
                        },
                    ]
                }
...
def phase6(tmp_path_factory) -> GlobalConfig:
    ...
    cfg = GlobalConfig.load(
        overrides={
            "project_name": "p6",
            "paths.root": str(ws),
            "pipeline": ["ingest", "load_genotypes", "selection", "demography", "pseudohaploid"],
            "logging.level": "WARNING",
            "stages.ingest.sample_sheet": str(sheet),
            "stages.ingest.callset": str(vcf),
