    ...
    def run(self, ctx: RunContext) -> StageResult:
        ...
        for pop, idx in groups.items():
            ...
            if callable_sites:
                ...
                row.update(
                    {"theta_w_per_site": None, "pi_per_site": None, "tajima_d": None, "Ne": None}
                )
            rows.append(row)

            folded = allel.sfs_folded(ac)
            for k, count in enumerate(folded):
                sfs_rows.append({"population": pop, "minor_allele_count": k, "n_sites": int(count)})
        ...
        limitations = (
            ...
            else "Verified callable-site denominator supplied."
        )
        art = Artifact(
